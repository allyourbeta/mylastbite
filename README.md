...
- `/` — public graph of last-meal times, with a 9 PM goal line, fasting-day markers, and a private-count daily Like button.
...
1. Provision a Neon Postgres database via the Vercel Marketplace integration — this sets `DATABASE_URL` automatically. Run the migration against it: `psql "$DATABASE_URL" -f db/migration.sql` (see `db/README.md`). It is safe to rerun and is required once after adding the daily Like feature. Existing deployments may instead run `db/add_daily_likes.sql`.
2. Generate a secret slug: `openssl rand -hex 12`. Set it as `LOG_SLUG` — the only place it's checked.
3. Deploy to Vercel and set `DATABASE_URL` and `LOG_SLUG` from `.env.example`, both server-side only. `api/meals.ts`, `api/log.ts`, and `api/likes.ts` deploy automatically as Vercel serverless functions.
